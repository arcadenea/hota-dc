...
//Dreamcast port: Do not use getopt.h (fixed entry)
#ifdef WIN32
	#include "getopt.h"
#endif
...
#ifdef DREAMCAST
	static char *QUICKSAVE_FILENAME = "/ram/quicksave";
	static char *RECORDED_KEYS_FILENAME = "/ram/recorded-keys";
#else
...
{
	SDL_Init(SDL_INIT_VIDEO|SDL_INIT_CDROM|SDL_INIT_AUDIO|SDL_INIT_JOYSTICK); //Add joystick Init
	...
	#ifdef DREAMCAST
   	 SDL_JoystickOpen(0);
	SDL_DC_MapKey(0,SDL_DC_START,SDLK_ESCAPE); //ESCAPE jumps sequences and exits!
	...
	SDL_DC_MapKey(0,SDL_DC_A,SDLK_z); //A:run / shoot
	SDL_DC_MapKey(0,SDL_DC_Y,SDLK_c); //X:jum
	SDL_DC_MapKey(0,SDL_DC_B,SDLK_x); //B:whip
	SDL_DC_MapKey(0,SDL_DC_X,SDLK_SPACE); //full-speed
	SDL_DC_MapKey(0,SDL_DC_R,SDLK_F5); //L
	SDL_DC_MapKey(0,SDL_DC_L,SDLK_F7); //R
	...
	if (nosound_flag == 0)
	{
		//Original game uses AUDIO_S8, 8000Hz and Mono. Perfect!
		#ifndef DREAMCAST
...
//Next two functions will not be used in DREAMCAST port :)
#ifndef DREAMCAST
static void help()
{
...
//DREAMCAST: Special init (ISO mode, no debug, no replays)
int main(int argc, char **argv)
{
	...
	fs_chdir("/cd/hota/");
	toggle_use_iso(1);
	fullscreen_flag = 0; //Fullscreen uses 304x192, which is rare.
	debug_flag = 0;
#else
	int options_index;
